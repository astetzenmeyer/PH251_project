...
#Exploring the median age for each county in California

mean_age_county <- ca_county_demographics %>%
group_by(name)%>%
summarize(med_age = mean(sum(med_age,na.rm = TRUE)))
...
total_pop_california <- ca_county_demographics %>%
group_by(name)%>%
summarize(total_pop = sum(pop2012, na.rm = TRUE))

#Exploring the total number of persons age 12 and above per County
