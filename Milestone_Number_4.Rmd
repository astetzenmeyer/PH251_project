...
means_by_county <- cov_vax_admin_2 %>% group_by(county) %>%
...
cov_vax_admin_4 <- cov_vax_admin_3 %>% 
  mutate(new_value = if_else(is.na(persons_fully_vaccinated),
                                    mean,
...
cov_vax_admin_5 <- cov_vax_admin_4 %>% group_by(county) %>%
  summarise(total_vax = sum(new_value),
